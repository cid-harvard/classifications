...
if __name__ == "__main__":
    ...
    df = pd.read_csv("./in/wikipedia-iso-country-codes.csv",
                     encoding="utf-8",
    ...
    en_names = pd.DataFrame.from_dict(dict(babel.Locale.parse("en_US").territories), orient="index")
    en_names.columns = ["name_en"]
    df = df.merge(en_names, left_on="code_alpha2", right_index=True, how="left")

    es_names = pd.DataFrame.from_dict(dict(babel.Locale.parse("es_419").territories), orient="index")
    ...
    region_codes = pd.read_table("./in/regions.tsv",
                                 dtype={"code":str})
